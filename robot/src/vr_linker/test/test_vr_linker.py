...
class TestVRLinker(TestCase):
    def test_vr_linker(self):
        ...
        self.assertFalse(vr_linker._is_vr_data_message({"pinch": 0}))

        self.assertTrue(vr_linker._is_vr_hand_message({"pinch": 0, "wrist": 0}))
        self.assertFalse(vr_linker._is_vr_hand_message({"x": 0}))
